...
class TestDataLeakage:
    ...
    def test_rolling_window_excludes_current_row(self) -> None:
        ...
        result = add_rolling_avg(
            df, "goals_scored", 3, ["team_id", "venue_type"], "rolling_avg_goals_3"
        )

        assert pd.isna(result.loc[0, "rolling_avg_goals_3"])  # row 0: no history yet
        assert result.loc[1, "rolling_avg_goals_3"] == 1.0  # row 1: mean([1]) = 1.0
        assert result.loc[2, "rolling_avg_goals_3"] == 1.5  # row 2: mean([1,2]) = 1.5
        assert result.loc[3, "rolling_avg_goals_3"] == 2.0  # row 3: mean([1,2,3]) = 2.0
        ...
        sentiment_df = pd.DataFrame(
            {
                ...
                "volume": [10] * 5,
                "captured_at": [datetime(2024, 1, 1, i, 0, tzinfo=timezone.utc) for i in range(5)],
            }
        )
        as_of = datetime(2024, 1, 1, 5, 0, tzinfo=timezone.utc)
        result = aggregate_sentiment_by_source(
            sentiment_df, "team1", time_window_hours=24, as_of=as_of
        )
    ...
    def test_biometric_uses_only_historical_readings(self) -> None:
        ...
        workload_df = pd.DataFrame(
            {
                "recorded_at": [now - timedelta(days=i) for i in range(30, 0, -1)],
